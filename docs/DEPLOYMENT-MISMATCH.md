...
The previous attempt to deploy the latest configuration was blocked by Vercel's account build-rate limit, not by an application build error. This update triggers one clean `main` production retry after that rate-limit period using the corrected auto-alias configuration.
